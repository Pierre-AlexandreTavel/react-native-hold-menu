...
import { HOLD_ITEM_TRANSFORM_DURATION, HOLD_ITEM_SCALE_DOWN_DURATION, HOLD_ITEM_SCALE_DOWN_VALUE, SPRING_CONFIGURATION, CONTEXT_MENU_STATE } from '../../constants';
...
const HoldItemComponent = ({
  ...
  const {
    deviceOrientation,
    windowHeight
  } = useDeviceOrientation(); //#endregion
  //#region variables
  ...
  const activateAnimation = ctx => {
    ...
    if (!ctx.didMeasureLayout) {
      ...
      if (!menuAnchorPosition) {
        const position = getTransformOrigin(measured.pageX, itemRectWidth.value, deviceOrientation === 'portrait' ? windowHeight : windowHeight, bottom);
