...
import { leftOrRight } from './calculations'; // const AnimatedView = Animated.createAnimatedComponent(View);
...
const MenuListComponent = () => {
  ...
  const AnimatedView = useMemo(() => (disableBlur === null || disableBlur === void 0 ? void 0 : disableBlur.value) === false ? Animated.createAnimatedComponent(BlurView) : Animated.View, [disableBlur]);
  ...
  const messageStyles = useAnimatedStyle(() => {
    const itemsWithSeparator = menuProps.value.items.filter(item => item.withSeparator);
    const translate = menuAnimationAnchor(menuProps.value.anchorPosition, menuProps.value.itemWidth, menuProps.value.items.length, itemsWithSeparator.length, menuWidth);
  ...
  const animatedInnerContainerStyle = useAnimatedStyle(() => {
    return {
      backgroundColor: theme.value === 'light' ? disableBlur ? 'rgba(255, 255, 255, 1)' : IS_IOS ? 'rgba(255, 255, 255, .75)' : 'rgba(255, 255, 255, .95)' : disableBlur ? 'rgba(255, 255, 255, 1)' : IS_IOS ? 'rgba(0,0,0,0.5)' : 'rgba(39, 39, 39, .8)'
    };
  }, [theme]);
