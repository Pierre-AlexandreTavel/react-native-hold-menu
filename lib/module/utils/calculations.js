...
import { MENU_TRANSFORM_ORIGIN_TOLERENCE, FONT_SCALE } from '../constants';
...
export const menuAnimationAnchor = (anchorPoint, itemWidth, itemLength, itemsWithSeparatorLength, menuWidth) => {
  'worklet';

  console.log('menuAnimationAnchor', {
    menuWidth
  });
  const MenuHeight = calculateMenuHeight(itemLength, itemsWithSeparatorLength);
  const splittetAnchorName = anchorPoint.split('-');
  const Center1 = itemWidth - 16;
  const Center2 = 0;
