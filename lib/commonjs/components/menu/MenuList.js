...
const MenuListComponent = () => {
  ...
  const {
    menuWidth
  } = (0, _hooks.useDeviceOrientation)();
  const AnimatedView = (0, _react.useMemo)(() => (disableBlur === null || disableBlur === void 0 ? void 0 : disableBlur.value) === false ? _reactNativeReanimated.default.createAnimatedComponent(_expoBlur.BlurView) : _reactNativeReanimated.default.View, [disableBlur]);
  ...
  const messageStyles = (0, _reactNativeReanimated.useAnimatedStyle)(() => {
    const itemsWithSeparator = menuProps.value.items.filter(item => item.withSeparator);
    const translate = (0, _calculations.menuAnimationAnchor)(menuProps.value.anchorPosition, menuProps.value.itemWidth, menuProps.value.items.length, itemsWithSeparator.length, menuWidth);

    const _leftPosition = (0, _calculations2.leftOrRight)(menuProps, menuWidth);
  ...
  const animatedInnerContainerStyle = (0, _reactNativeReanimated.useAnimatedStyle)(() => {
    return {
      backgroundColor: theme.value === 'light' ? disableBlur ? 'rgba(255, 255, 255, 1)' : _constants.IS_IOS ? 'rgba(255, 255, 255, .75)' : 'rgba(255, 255, 255, .95)' : disableBlur ? 'rgba(255, 255, 255, 1)' : _constants.IS_IOS ? 'rgba(0,0,0,0.5)' : 'rgba(39, 39, 39, .8)'
    };
  }, [theme]);
