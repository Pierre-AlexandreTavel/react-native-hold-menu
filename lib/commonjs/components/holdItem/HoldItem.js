...
const HoldItemComponent = ({
  ...
  const {
    deviceOrientation,
    windowHeight
  } = (0, _hooks.useDeviceOrientation)(); //#endregion
  //#region variables
  ...
  const activateAnimation = ctx => {
    ...
    if (!ctx.didMeasureLayout) {
      ...
      if (!menuAnchorPosition) {
        const position = (0, _calculations.getTransformOrigin)(measured.pageX, itemRectWidth.value, deviceOrientation === 'portrait' ? windowHeight : windowHeight, bottom);
